...
export async function fetchApps(
  ...
  try {
    ...
    const data = await get<AppsResponse>(`/console/api/apps?${queryParams.toString()}`);
 ...
 * @returns API 密钥列表
 */
export async function fetchAppApiKeys(appId: string): Promise<ApiKeysResponse | null> {
  try {
    const domain = getApiDomain();
    ...
    console.log(`正在从 ${domain}/console/api/apps/${appId}/api-keys 获取 API 密钥...`);
    
    // 使用 http-client 的 get 方法
    const data = await get<ApiKeysResponse>(`/console/api/apps/${appId}/api-keys`);
 ...
 * @returns 是否删除成功
 */
export async function deleteAppApiKey(appId: string, keyId: string): Promise<boolean> {
  try {
    const domain = getApiDomain();
...
export async function fetchAppDetail(appId: string): Promise<AppData | null> {
  try {
    ...
    if (!response.ok) {
      throw new Error(`获取应用详情失败: ${response.status} ${response.statusText}`);
    }
...
export async function fetchAppStatistics(
  ...
  try {
    ...
    console.log(`正在从 ${domain}/console/api/apps/${appId}/statistics/${type} 获取统计信息...`);
 ...
 * @returns 应用配置数据
 */
export async function exportApp(appId: string, includeSecret: boolean = false): Promise<{ data: string } | null> {
  try {
    const domain = getApiDomain();
 ...
 * @returns 新应用数据
 */
export async function copyApp(appId: string, data: {
  name: string;
  icon_type: string;
  icon: string;
  icon_background: string;
