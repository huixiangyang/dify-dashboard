...
export default function AppStatistics({ appId, days = 7 }: AppStatisticsProps) {
    ...
    const loadStatistics = async () => {
      ...
      try {
        ...
        const [
          ...
          fetchAppStatistics(
            appId,
            StatisticsType.DAILY_CONVERSATIONS,
            startFormatted,
            endFormatted,
          ),
          fetchAppStatistics(
            appId,
            StatisticsType.DAILY_END_USERS,
            startFormatted,
            endFormatted,
          ),
          fetchAppStatistics(
            appId,
            StatisticsType.AVERAGE_SESSION_INTERACTIONS,
            startFormatted,
            endFormatted,
          ),
          fetchAppStatistics(
            appId,
            StatisticsType.TOKENS_PER_SECOND,
            startFormatted,
            endFormatted,
          ),
          fetchAppStatistics(
            appId,
            StatisticsType.USER_SATISFACTION_RATE,
            startFormatted,
            endFormatted,
          ),
          fetchAppStatistics(
            appId,
            StatisticsType.TOKEN_COSTS,
            startFormatted,
            endFormatted,
          ),
          fetchAppStatistics(
            appId,
            StatisticsType.DAILY_MESSAGES,
            startFormatted,
            endFormatted,
          ),
        ]);
        ...
        // 设置数据
        if (conversationsRes)
          setConversations((conversationsRes as StatisticsResponse).data);
        if (endUsersRes) setEndUsers((endUsersRes as StatisticsResponse).data);
        if (avgInteractionsRes)
          setAvgInteractions((avgInteractionsRes as StatisticsResponse).data);
        if (tokensPerSecondRes)
          setTokensPerSecond((tokensPerSecondRes as StatisticsResponse).data);
        if (satisfactionRateRes)
          setSatisfactionRate((satisfactionRateRes as StatisticsResponse).data);
        ...
        chart = (
          <ResponsiveContainer height={200} width="100%">
            ...
            >
              ...
              <YAxis tickFormatter={valueFormatter} width={40} />
              <Tooltip
        ...
        <div className="mb-4 grid grid-cols-3 gap-4">
          <div className="bg-default-50 p-3 rounded text-center">
            ...
            <p className="font-medium">
              {tokenCosts.total_tokens?.toLocaleString() || "0"}
            </p>
          </div>
          <div className="bg-default-50 p-3 rounded text-center">
            <p className="text-xs text-default-500">总成本</p>
            <p className="font-medium">
              ${tokenCosts.total_cost?.toFixed(4) || "0.0000"}
            </p>
          </div>
          <div className="bg-default-50 p-3 rounded text-center">
            <p className="text-xs text-default-500">平均成本/1K Token</p>
            <p className="font-medium">
              ...
              {tokenCosts.total_tokens > 0
                ? (
                    tokenCosts.total_cost /
                    (tokenCosts.total_tokens / 1000)
                  ).toFixed(4)
                : "0.0000"}
            </p>
          </div>
  ...
  return (
    <div className="my-6">
      ...
      <div className="mb-6">{renderTokenCosts()}</div>
    </div>
  );
}
